...
var parse = CanvasXmlV3Parser.parse = function(xml, options) {
    ...
    createdDiv.innerHTML = xml
    let root = createdDiv.firstChild
    console.log(Snap(root).attr('width'))
    if (!xml) {
    ...
    canvas['width'] = parseFloat(Snap(root).attr('width'));
    canvas['height'] = parseFloat(Snap(root).attr('height'));
    canvas['viewBox'] = parseViewBox(Snap(root).attr('viewBox').vb);
    canvas['layerList'] = [];
    canvas['svgNode'] = root;
    let allDomG = Array.prototype.slice.call(root.querySelectorAll('g'))
    allDomG.forEach(function(el, i) {
        var layer = {};
        var group = Snap(el);
        var fragment = Snap.parse(el);
        var newId = group.attr('id');
        var nID = '';
        ...
        if (!!group.select('text')) {
            ...
            // }           
            Object.assign(layer, parseTextBox(el));
        } else if (!!group.select('image')) {
            layer['type'] = 'Image';
            Object.assign(layer, parseImage(el, baseUrl));
        } else if (group.hasClass('background') || group.attr('id') === 'background') {
            layer['type'] = 'Background';
            Object.assign(layer, parseBackground(el));
        } else {
            layer['svgNode'] = el;
        }
...
var attr = function(el, key, defaultValue) {
    var styleString = Snap(el).attr('style');
    var styleMap = styleString && styleString.split(';').map(
        function(e, i) {
            var tuple = e.split(':');
    ...
    return styleMap[key] || Snap(el).attr(key) || defaultValue || undefined;
};

var parseViewBox = function(viewBoxString) {
    var array = (viewBoxString || '').split(/[,\s]+/).map(parseFloat);
...
export default CanvasXmlV3Parser;
// });