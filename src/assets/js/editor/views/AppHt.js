...
require(['jquery', 'underscore', 'backbone', 'data/ExternalDataLoader',
        'data/CanvasXmlV5Parser', 'domain/editor', 'domain/portfolio',
        'domain/creativework', 'domain/graphics/canvas', 'domain/graphics/layerfactory', 'views/EditorPageHt', 'widgets/customwidgets'
    ],
    function($, _, Backbone, ExternalDataLoader,
        ...
        $.ajax({
            ...
            if (data.code == 1) {
                ...
                if (dlist && dlist.length && dlist[0].aiText) {
                    $('.batchTextCheckBox .jad-checkbox-wrapper').trigger('click')
                }
                ...
                //sessionStorage.setItem('templateSizeList', JSON.stringify(dlist))
        ...
        function run(portfolio, dataList, token) {
            ...
            var whiteSkuPicUrl = portfolio.get('whiteSkuPicUrl')
            _(creativeWorks).forEach(function(creativeWork) {
                ...
                if (fileUrl) {
                    ...
                    ExternalDataLoader.load(fileUrl, CanvasXmlV3Parser,whiteSkuPicUrl)
