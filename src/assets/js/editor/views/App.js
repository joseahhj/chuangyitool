...
require(['jquery', 'underscore', 'backbone','data/ExternalDataLoader', 
	'data/CanvasXmlV4Parser', 'domain/editor', 'domain/portfolio', 
	'domain/creativework', 'domain/graphics/canvas', 'domain/graphics/layerfactory', 'views/EditorPage','widgets/customwidgets'],
	function($, _, Backbone,  ExternalDataLoader, 
		CanvasXmlV4Parser, Editor, Portfolio, CreativeWork, Canvas, LayerFactory, EditorFrame){
		...
		function run(portfolio, dataList, token){		
			...
			var AppRouter = Backbone.Router.extend({
			    ...
			    reset: function() {
			        ...
			        this.navigate('edit?creative=' + size, {replace: true, trigger: true});
			    },
				renderList: function(){
					var _this = this;
			...
			_(creativeWorks).forEach(function(creativeWork) {
		      ...
		      var onCanvasRejected_ = function(errorMessage) {
		        ...
		        if (fileUrl) {
		            ...
		            // //ExternalDataLoader.load(data.content.templateSizeList[0].svgInfo, CanvasXmlV4Parser);					
							
		            ExternalDataLoader.load(fileUrl, CanvasXmlV4Parser)
